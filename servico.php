<?php
	...
	$sqlprodutor = 'SELECT id, nome FROM produtores ORDER BY nome';
	$sqltipo = 'SELECT id, titulo FROM tipos ORDER BY titulo';
	...
	if (empty($_POST)) {
		...
		if (empty($id)) 
		{
			...
			$q = $pdo->prepare($sql);
			$q->execute(array($servico));
			
			$id = $pdo->lastInsertId();
			
		}else{
			$sql = "update servicos set servico=? where id=?";
			$q = $pdo->prepare($sql);
			$q->execute(array($servico, $id));
		}
		...
		<div class="col-md-8">
			<div class="card card-user">
				<div class="card-header">
					<h5 class="card-title">Servi&ccedil;os</h5>
				</div>
				<div class="card-body">
					<form method="post">
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<label>Nome Servi&ccedil;o:</label> 
									<input class="form-control" name="servico" value="<?php echo !empty($servico)?$servico:'';?>" required="required" placeholder="Nome do Servi&ccedil;o">
						...
						<div class="row">
							<div class="update ml-auto mr-auto">
								...
								<a href="servicos.php" class="btn btn-danger btn-round">Cancelar</a>
							</div>
						</div>
					</form>
				</div>
