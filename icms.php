...
if (empty($_POST)) {
	...
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	if (empty($id)) {
		$sql = "INSERT INTO icms (icms) VALUES(?)";
		$q = $pdo->prepare($sql);
		$q->execute(array($icms));

		$id = $pdo->lastInsertId();
	} else {
		$sql = "update icms set icms=? where id=?";
		$q = $pdo->prepare($sql);
		$q->execute(array($icms, $id));
	}
                    ...
                    <form method="post">
                        ...
                        <div class="row">
                            <div class="update ml-auto mr-auto">
                                ...
                                <a href="icms.php" class="btn btn-danger btn-round">Cancelar</a>
                            </div>
                        </div>
                    </form>
                </div>
