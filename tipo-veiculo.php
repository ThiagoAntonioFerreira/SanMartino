...
if (empty($_POST)) {
	...
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	if (empty($id)) {
		$sql = "INSERT INTO tipo_veiculos (tipo_veiculo) VALUES(?)";
		$q = $pdo->prepare($sql);
		$q->execute(array($tipo_veiculo));

		$id = $pdo->lastInsertId();
	} else {
		$sql = "update tipo_veiculos set tipo_veiculo=? where id=?";
		$q = $pdo->prepare($sql);
		$q->execute(array($tipo_veiculo, $id));
	}
                    ...
                    <form method="post">
                        ...
                        <div class="row">
                            <div class="update ml-auto mr-auto">
                                ...
                                <a href="tipo_veiculos.php" class="btn btn-danger btn-round">Cancelar</a>
                            </div>
                        </div>
                    </form>
                </div>
